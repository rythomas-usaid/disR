...
correct_headcount <- function(x) {
  ...
  select(-uic) %>% pivot_wider(names_from = ic, values_from = value) %>%
...
```{r}
# Max commodity participant producers
...
sum_activity_product <- sex_indicators %>%
  filter(str_detect(uid, "EG.3.2-26/EG.3.2-x19_product_number") &
           str_detect(disag3, " Smallholder")) %>%
  left_join(x = ., y = filter(values, type == "Actual"),
            by = join_by(id == id_ind),
            suffix = c("_ind", "_val"), # Label variables
  ...
  left_join(x = ., y = ims,
            by = join_by(id_im == id)) %>%
  group_by(ro, ou, year) %>%
  summarise(sum_activity_product = sum_(value))

sum_activity_recipients <- sex_indicators %>%
  filter(str_starts(uic, "EG.3.2-27/EG.3.2-x6") & str_ends(uid, "_number")) %>%
  left_join(x = ., y = filter(values, type == "Actual"),
            by = join_by(id == id_ind),
            suffix = c("_ind", "_val"), # Label variables
            keep = FALSE) %>%
  ...
  left_join(x = ., y = ims,
            by = join_by(id_im == id)) %>%
  group_by(ro, ou, year) %>%
  summarise(sum_activity_recipients = sum_(value))

headcount_ou %>% set_disaggs()  %>% filter(d2 == "Total")
```
