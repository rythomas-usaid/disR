...
check_1_d <- check_1_c %>% group_by(`Activity Name`, `Activity Code`)
...
#need to figure out how to select sex disaggregates only
check_3 <- data3 %>% filter(`Collection Period Name` == "FY2023-Actual") %>% 
                    ...
                    mutate(disaggregate_type = case_when(UDN == "3.1.1" ~ "sex",
                                                         UDN == "3.1.2" ~ "sex",
                                                         UDN == "3.1.3" ~ "sex",
                                                         UDN == "3.1.4" ~ "sex",
                                                         UDN == "3.1.5" ~ "sex")) %>% 
  #in instances when UDN starts with 3.1 assign disaggrgate_type "sex" everything else will be NA
#drop NAs, we are left with only the sex disaggregates for each indicator
                     drop_na(disaggregate_type)

check_3_a <- check_3 %>% group_by(`Activity Code`, `Indicator Code`, `Disaggregate Country`) %>% mutate(group_id = cur_group_id()) %>%
  ungroup() #group by indicator and assign and id to each unique activity/indicator combo

  
#now pivot wider based on that group id 
check_3_b <- check_3_a %>% pivot_wider(names_from = 'Disaggregate Name',
                                       values_from = Actual_value)  

rm(check_3, check_3_a)
...
```{r check 5: is EG.3.2-24 less than EG.3-2?}
check_5 <- data3 %>% filter(`Indicator Code` == "EG.3.2-24" | `Indicator Code` == "EG.3-2",
...
#check for dupes
check <- get_dupes(check_5, `Activity Code`, `Indicator Code`, `Fiscal Year`, `Disaggregate Country`)

rm(check)

check_5_a <- check_5 %>% group_by(`Activity Code`, `Disaggregate Country`) %>%
                         pivot_wider(names_from = `Indicator Code`,
                                     values_from = Actual_value)

check <- get_dupes(check_5_a, `Activity Code`, `Fiscal Year`, `Disaggregate Country`)

rm(check)

check_5_b <- check_5_a %>% mutate(issue = case_when(`EG.3.2-24` > `EG.3-2`~ "EG.3.2-24 has higher reported value than EG.3-2",
                                          !is.na(`EG.3.2-24`) & is.na(`EG.3-2`) ~ "EG.3.2-24 reported, EG.3-2 not reported")) %>% drop_na(issue)

rm(check_5, check_5_a)

```

```{r check 6: EG.3.2-2 flag if “continuing” disaggregate for FY[reporting year] is greater than “new” FY[last reporting year] count}
...
check_6 <- data3 %>% filter(`Indicator Code` == "EG.3.2-2",
...
check_6_a <- check_6 %>% group_by(`Activity Code`) %>% 
                        pivot_wider(names_from = 
                                   c(`Fiscal Year`, `Disaggregate Name`),
                                   values_from = Actual_value)

check <- get_dupes(check_6_a, `Activity Name`, `Activity Code`, `Indicator Code`, `Disaggregate Country`) #make sure there is only one row per activity

#check for when this years continuing is greater than last years new
#2023_continuing > 2022_new
check_6_b <- check_6_a %>% mutate(issue = case_when(`2023_Continuing` > `2022_New` ~ "This year's continuing > last years new")) %>% drop_na(issue)

rm(check_6, check_6_a, check)
```


```{r check 7: Is volume of sales reported under EG.3.2-26 total sales equal to or greater than total production reported under EG.3-10,-11,-12 yields.}
...
check_7_a <- full_join(check7_yield, check7_sales, 
...
rm(check_7, check7_sales, check7_yield)
```

```{r check 8: Flag if yields (sum of all yield indicators by sex) are exactly the same for men and women, this is a red flag.}
...
check_8_a <- full_join(check8_female, check8_male, 
...
rm(check_8, check8_female, check8_male)
```


```{r check 9 :EG.3-2_OULevel should be equal to or less than EG.3-2 summed across IMs}
...
check_9_a <- check9_ou %>% left_join(check9_im) %>%
...
rm(check_9, check9_im, check9_ou)
```

```{r check 11: If EG.3.2-27, EG.4.2-7 or EG.10.10-7 are present then YOUTH-3 and GNDR-2 should be present And vice versa,}
...
check_11_2 <- check_11 %>% filter(`Indicator Code` == `Disaggregate Code` |`Disaggregate Code` == "Youth-3") 

#pivot wider so we have one row per activity
check11_wide <- check_11_2 %>%
  group_by(`Activity Code`, `Activity Name`) %>% 
  ...
  mutate(issue = case_when((!is.na("EG.3.2-27") & is.na("EG.4.2-7") & 
                               ...
                               Youth-3 may be missing") 


#Of the remaining  activities, how many activities reported on either EG.3.2-27 or EG.4.2-7 AND both Youth-3 and GNDR-2
check12_output <- check12_incomplete %>%
  filter(flag == "missing complete set of indicators") %>% 
  select(pa_id, indicator, actual2022, flag) # Filter activities that are missing complete set of indicators

```

```{r check 12: Flag if GNDR-2 and YOUTH-3 denominators are larger than the total for EG.3-2}
#select columns we need and only the current year of data
check_12 <- data3 %>% select('Activity Code', 'Activity Name', 
                             'Indicator Code', 'Disaggregate Name', 'Disaggregate Country',
                             'Fiscal Year', 'Collection Period Name', Actual_value) %>% 
                      filter(`Collection Period Name` == "FY2023-Annual")

# Create dataframe with just Youth-3 and GNDR-2 denominator totals 
check_12_gy <- check_12 %>% 
               select('Activity Code', 'Activity Name','Indicator Code', 
...
%>%  #ML Left off here need to check data entry cells may need to do this at the sex disagg individual level
  

              pivot_wider(id_cols = c()
              names_from = indicator,
              values_from = actual2022)

# Create dataframe with just EG.3-2 total
check13_32 <- data8 %>%
  filter(str_detect(indicator_name, "\\bEG.3-2:Number of individuals participating in USG food security programs")) %>% 
  select(pa_id, actual2022) %>%
  rename(`EG.3-2_sum` = actual2022)

# Join and compare Youth/GNDR denom totals to EG3-2 total per activity
check13_output <- full_join(check13_GY, check13_32, by = "pa_id") %>%
  #select(-indicator_name) %>%
  mutate(flag = case_when(`GNDR-2` > `EG.3-2_sum` ~ "Review: GNDR-2 and/or Youth-3 are greater than EG.3-2.",
                          `Youth-3` > `EG.3-2_sum` ~ "Review: GNDR-2 and/or Youth-3 are greater than EG.3-2.",
                          is.na(`EG.3-2_sum`) ~ "Review: EG.3-2 NOT reported")) %>% 
  drop_na(flag) # This removes observations where EG3-2 and genderyouth are both NA AND it removes obs where genderyouth is NA but EG3-2 is inputted. OUs do not need to input data for genderyouth, but can still report on EG3-2 


```

```{r check 13: Flag if EG.10.4-8 actuals are larger than 10.4-7}

check_13 <- data3 %>% select('Activity Code', 'Activity Name', 'Disaggregate Country',
...
```{r check 14: Flag if total units of production (EG.3-10,11,12) is lower than the number of hectares applied (EG.3.2-25) for a respective commodity}

#select the columns we need
check_14 <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual") %>% 
...
check14_a <- full_join(check14_25, check14_yield, 
...
rm(check14_25, check_14, check14_yield)
```

```{r check 15: New! flag any activity that reported sex entirely DNA} }
...
```{r check 16: New! flag any activity that didn't report EG.3-2 }
...
check_16_all_activities <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual") %>% distinct(`Activity Name`, `Activity Code`)  #filter for all activities that appear at all with FY23 rows

check_16_eg_reported <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual", `Indicator Code` == "EG.3-2") %>% distinct(`Activity Name`, `Activity Code`, `Indicator Code`) #filter for activities reporting on EG.3-2, create 1 row for each

#create a df of those that appear in all activities but not in eg reported
check_16 <- check_16_all_activities %>% anti_join(check_16_eg_reported)

check_16_a <- check_16[!grepl("_HLI", check_16$`Activity Name`),] %>% #drop anywhere that activity name is _HLI since these are HLi activities that only report EG.3-2OULevel
              mutate(issue = "Activity did not report on EG.3-2")

rm(check_16_all_activities, check_16_eg_reported, check_16)          
```

```{r check 17: New! If HL.8.3-3 #institutions can't exceed number of new insitutions}

check_17 <- data3 %>% filter(`Indicator Code` == "HL.8.3-3") %>% filter(`Collection Period Name` == "FY2023-Annual") %>% 
...
check_17_a <- check_17 %>% mutate(type = case_when(`Indicator Code` == `Disaggregate Code` ~ "indicator total",
                                  `Indicator Code` != `Disaggregate Code` ~ "disaggregate"))

check_17_total <- check_17_a %>% filter(type == "indicator total")

#filter for new disagg and mutate disagg_type == new
check_17_new <- check_17_a %>% filter(type == "disaggregate") %>% filter(`Disaggregate Name` == 
...
check_17_b <- rbind(check_17_total, check_17_new) %>% #bind
...
rm(check_17, check_17_a, check_17_new, check_17_total)
```

```{r check 18: New! Is HL.8.4-3 a reasonable dollar amount?}
...
check_18 <- data3 %>% filter(`Indicator Code` == "HL.8.4-3", `Collection Period Name` == "FY2023-Annual") %>% filter(`Indicator Code` == `Disaggregate Code`) %>% mutate(issue = case_when(Actual_value < 15 ~ "Super low value for dollar amount, may need review")) %>% drop_na(issue)

```


```{r check 19: New! Did an activity report Climate adapation disagg for 3.2-24 or 3.2-25? Consider reporting on 11-6 in the PPR}
...
check_19_3s <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual") %>%
                      select(`Activity Code`, `Activity Name`, `Indicator Code`,
                             `Disaggregate Name`, Actual_value) %>% 
...
check_19_11 <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual") %>%
                         select(`Activity Code`, `Activity Name`, `Indicator Code`,
...
check19_a <- full_join(check_19_3s, check_19_11, by = c("Activity Code", "Activity Name")) %>%
...
rm(check_19, check_19_11, check_19_3s)
```

```{r check 20: New- Ukr supp! If an activity reported on Urk supp indicators quarterly & annually, is the annual value jsut the sum of the quarters? flag if so}
...
#for this year only
check_20 <- data3 %>% filter(`Indicator Code` == `Disaggregate Code`, 
...
#create an annual df
check20_annual <- check_20 %>% filter(`Collection Period Name` == "FY2023-Annual") %>%
                               rename("Indicator Code Annual" = "Indicator Code")

#create an all reporting but annual df
check20_qrt <- check_20 %>% filter(`Collection Period Name` != "FY2023-Annual") %>% 
                            group_by(`Activity Code`, `Activity Name`, `Indicator Code`) %>%
...
check20_a <- full_join(check20_qrt, check20_annual, by = c("Activity Code", "Activity Name", 
...
rm(check_20, check20_annual, check20_qrt)
```
...
```{r check 24: New! Check if there is any reporting in FY24 or FY25 actuals.}
check_25 <- data3 %>% select(`Collection Period Name`, `Activity Code`, `Disaggregate Code`,
...
```{r check 26: Also flag for number of individuals applying (EG.3.2-24): can be lower than what is reported in yield but not higher}

#select the columns we need
check_26 <- data3 %>% filter(`Collection Period Name` == "FY2023-Annual") %>% 
  select(`Collection Period Name`, `Activity Name`, `Activity Code`, `Indicator Code`, `Disaggregate Country`, `Disaggregate Commodity`, `Disaggregate Name`, Actual_value)

# Create dataframe for just EG.3.10-11-12 select commodity values (units of production)
check26_yield <- check_26 %>%
  filter(`Indicator Code` %in% c('EG.3-10-11-12_CROP', 'EG.3-10-11-12_DAIRY', 'EG.3-10-11-12_EGGS', 'EG.3-10-11-12_PONDAQ', 'EG.3-10-11-12_LIVSTK', 'EG.3-10-11-12_OTHER')) %>%
  filter(`Disaggregate Name` == "Units of Production by Sex") %>% #units of production disaggs should all equal but sex is required so we'll use that value 
  group_by(`Activity Name`, `Activity Code`, `Indicator Code`, `Disaggregate Commodity`) %>%
  summarise(yield_sum = sum(Actual_value))
...
#join our yield data to our hectare data, here we do a full join so even those commodities that don't appear in both dfs will appear in our final df
check26_a <- full_join(check26_24, check26_yield, 
                       by = c("Activity Code", "Activity Name", "Disaggregate Commodity")) %>% 
...
rm(check_26, check26_24, check26_yield)
```
