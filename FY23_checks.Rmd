...
check_6_a <- check_5 %>% group_by(`Activity Code`, `Disaggregate Country`) %>%
...
check_13_2 <- check_11 %>% filter(`Indicator Code` == `Disaggregate Code` |`Disaggregate Code` == "Youth-3") 

#pivot wider so we have one row per activity
check13_wide <- check_13_2 %>%
  group_by(`Activity Code`, `Activity Name`) %>% 
...
check13_output <- check13_incomplete %>%
...
check20_a <- full_join(check_20_gy, check20_32, by = c("Activity Code", "Activity Name")) %>%
  ...
  mutate(flag = case_when(`Actual_value_gy` > `Actual_value EG.3-2` ~ "Review: GNDR-2 and/or Youth-3 are greater than EG.3-2.",
