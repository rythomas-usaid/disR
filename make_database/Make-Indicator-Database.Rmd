  ...
  mutate(ic_1819 = 
             if_else(ic %in% c("EG.3.2-25","EG.3.2-x18")
              ...
              , if_else(ic %in% c("EG.3.1-14/-15","EG.3.2-x22")
                        ...
                        , ic))))),
             .after = ic)  %>%
  mutate(disagg_group = paste(d1, d2, d3, d4, sep=" / ")) %>%
  mutate(indicator_name = case_when(
    if_any(everything(), ~ str_detect(., "Value of Sales")) ~ paste0(ic_1819, "_value")
    , if_any(everything(), ~ str_detect(., "Number of Participants")) ~ paste0(ic_1819, "_number")
    , if_any(everything(), ~ str_detect(., "Volume of Sales")) ~ paste0(ic_1819, "_volume")
    , .default = ic_1819))


# # sex disaggs in d1
# c("EG.11-x6", "EG.3.1-x13", "EG.3.1-x22", )
# # sex disaggs in d2
# c("EG.3-2", "EG.3-2_OULevel", "EG.3-x1", "EG.3-x6, -7, -8", "	EG.3-x9", "EG.3.2-2",
#   )
# # sex disaggs in d3
# c("EG.10.4-7", "EG.10.4-8", "EG.3.2-24/EG.3.2-x17", "EG.3.2-25/EG.3.2-x18", 
#   )
# # sex disaggs in d4
# c("EG.3.2-26/EG.3.2-x19", )
...
# Write to files
```{r}
write.xlsx(indicators_df, "../database/magnus/indicators_df.xlsx")
write.xlsx(indicators, "../database/magnus/indicators.xlsx")
write.xlsx(values, "../database/magnus/values.xlsx")
write.xlsx(ims, "../database/magnus/ims.xlsx")
write.xlsx(ftf_target_countries, "../database/magnus/ftf_target_countries.xlsx")


save(indicators_df, values, ims, indicators, ftf_target_countries, file = "../database/magnus/indicators_db.Rdata")

save(indicators_df, indicators, values, ims, ftf_target_countries, file = "../disR/data/magnus.Rdata")
```
