#' Combine the raw data to a database
#'
#'
#' @import tidyverse
#' @import openxlsx
...
indicators_df <- map(str_c(path, indicator_files), ~ read_indicator(.x)) %>%
  list_rbind() %>%
...
ftf_target_countries <- data.frame(country = c("Bangladesh","Democratic Republic of the Congo",
                                               "Ethiopia","Ghana",
                                               "Guatemala","Honduras",
                                               "Kenya","Liberia",
                                               "Madagascar","Malawi",
...
# Write to files
write.xlsx(indicators_df, "data/indicators_df.xlsx")
write.xlsx(indicators, "data/indicators.xlsx")
write.xlsx(values, "data/values.xlsx")
write.xlsx(ims, "data/ims.xlsx")
write.xlsx(ftf_target_countries, "data/ftf_target_countries.xlsx")

save(indicators_df, values, ims, indicators, ftf_target_countries, file = "data/indicators_db.Rdata")
